...
*/
    ...
    static function FOEReferralsWithRewardsOnly(oSession: Session) {
        if (oSession.uriContains("forgeofempires.com")) {
            var oResponseBody = oSession.GetResponseBodyAsString();
            var oJSON = Fiddler.WebFormats.JSON.JsonDecode(oResponseBody).JSONObject;
            try {
                var changed = false;
                for (var elem in oJSON) {
                    if (elem["requestClass"] == "FriendService") {
                        var invGroups = new System.Collections.ArrayList();
                        if (elem["requestMethod"] == "startup") {
                            invGroups = elem["responseData"]["invitation_reward_groups"];
                        } else if (elem["requestMethod"] == "updateInvitationRewardGroups") {
                            invGroups = elem["responseData"];
                        }
                        
                        for (var g in invGroups) {
                            var newFriends = new System.Collections.ArrayList();
                            for (var f in g["friends"]) {
                                if (f['progress'] >= 100) {
                                    newFriends.Add(f);
                                }
                            }
                            g["friends"] = newFriends;
                        }
                        changed = true;
                    }
                }
                if (changed) {
                    oSession.utilSetResponseBody(Fiddler.WebFormats.JSON.JsonEncode(oJSON));                    
                }
            } catch (e) {
                FiddlerApplication.Log.LogString(e);
            }
        }
    }
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        FOEReferralsWithRewardsOnly(oSession);
    }
